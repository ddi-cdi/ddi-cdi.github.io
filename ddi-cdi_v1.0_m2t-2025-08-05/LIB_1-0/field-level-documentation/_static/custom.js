...
// remove markup in table cells
$('.datatable-associations').find('th a').contents().unwrap();
$('.datatable-associations').find('th > p').contents().unwrap();
$('.datatable-associations').find('td > p').contents().unwrap();
$('.datatable-associations').find('colgroup').remove();
$('.datatable-associations').DataTable({
	autoWidth: false,
	columnDefs: [
		{ targets: [0, 3, 6, 7], className: 'dt-body-center' }
	],
	layout: {
		responsive: true,
		top: 'searchPanes'
